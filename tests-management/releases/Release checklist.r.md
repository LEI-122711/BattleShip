# Release checklist

## Testing checklists

### S1 Release checklist [ID: TMS-S1]
* [passou] C1 Reports [ID: TMS-S1-C1] @Bianca


### S2 Automated tests checklist [ID: TMS-S2]
* [passou] C2 Unit tests [ID: TMS-S2-C2] @Bianca


## Unit tests

### S3 Ships test case [ID: TMS-S3]
* [passou] C3 BargeTest [ID: TMS-S3-C3] @Bianca
    tags: #unit-tests, #test-cases

* [passou] C4 CaravelTest [ID: TMS-S3-C4] @Bianca
    tags: #unit-tests, #test-cases

* [passou] C5 CarrackTest [ID: TMS-S3-C5] @Bianca
    tags: #unit-tests, #test-cases

* [passou] C6 FrigateTest [ID: TMS-S3-C6] @Beatriz
    tags: #unit-tests, #test-cases

* [passou] C7 GalleonTest [ID: TMS-S3-C7] @Beatriz
    tags: #unit-tests, #test-cases

* [passou] C8 ShipTest [ID: TMS-S3-C8] @Beatriz
    tags: #unit-tests, #test-cases


### S4 Core Components test case [ID: TMS-S4]
* [passou] C9 PositionTest [ID: TMS-S4-C9] @Beatriz
    tags: #unit-tests, #test-cases

* [passou] C10 CompassTest [ID: TMS-S4-C10] @Beatriz
    tags: #unit-tests, #test-cases


### S5 Tasks test case [ID: TMS-S5]
* [passou] C11 TasksTest [ID: TMS-S5-C11] @Andre
    tags: #unit-tests, #test-cases

* [passou] C12 GameTest [ID: TMS-S5-C12] @Andre
    tags: #unit-tests, #test-cases

* [passou] C13 FleetTest [ID: TMS-S5-C13] @Andre
    tags: #unit-tests, #test-cases


### S6 Global test case [ID: TMS-S5]
* [passou] C14 BattleshipTest [ID: TMS-S5-C14] @Andre
    tags: #unit-tests, #test-cases


